...
from dspm.loop.watch import fetch_text
...
def discover_pypi(
    *,
    ...
    if fixture:
        ...
        for row in rows:
            ...
            parsed = parse_project(
                ...
                pypi_name=name,
                module=str(row.get("module") or "dspm/classification"),
            )
            if parsed:
                found.append(parsed)
        return found
    ...
    for item in load_catalog(catalog_path):
        ...
        parsed = parse_project(json.loads(body), pypi_name=name, module=str(item.get("module") or "dspm/audit"))
        if parsed:
            found.append(parsed)
    return found
