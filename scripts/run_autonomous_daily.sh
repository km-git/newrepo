...
export EW_OKF_BRAIN_DIR="${EW_OKF_BRAIN_DIR:-/tmp/ew-okf-autonomous}"
...
echo "=== Phase 2: improvement + OKF learn ==="
"$PY" -c "
import json
from engine.improvement_cycle import run_improvement_cycle
r = run_improvement_cycle(is_crypto=True, persist_okf=True)
print(json.dumps({
...
echo "=== Phase 5: web intel + social research ==="
"$PY" -c "
...
soc = run_social_strategy_validation(use_llm=False)
