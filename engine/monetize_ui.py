...
STATIC_EXPLORER_PATH = Path("reports/monetize_explorer.html")
...
def build_explorer_state(tier: Optional[str] = None) -> Dict[str, Any]:
    ...
    state: Dict[str, Any] = {
        ...
        "cli": {
            ...
            "static": "python3 ew_tool.py --monetize-ui --static",
...
def _embed_json(payload: Any) -> str:
    """JSON safe to drop inside a <script type=application/json> tag."""
    return json.dumps(payload, default=str, separators=(",", ":")).replace("<", "\\u003c")


def render_monetize_html(state: Optional[Dict[str, Any]] = None) -> str:
    """Self-contained explorer HTML with the access matrix embedded for file:// use."""
    payload = state if state is not None else build_explorer_state()
    snippet = (
        f'<script type="application/json" id="embedded-state">{_embed_json(payload)}</script>\n'
        "</head>"
    )
    if "</head>" not in MONETIZE_HTML:
        raise ValueError("MONETIZE_HTML is missing </head>")
    return MONETIZE_HTML.replace("</head>", snippet, 1)


def write_monetize_html(
    output_dir: str = DEFAULT_OUTPUT_DIR,
    state: Optional[Dict[str, Any]] = None,
) -> Path:
    path = Path(output_dir) / "monetize.html"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(render_monetize_html(state), encoding="utf-8")
    return path


def write_static_monetize_html(
    path: Optional[str] = None,
    state: Optional[Dict[str, Any]] = None,
) -> Path:
    dest = Path(path) if path else STATIC_EXPLORER_PATH
    dest.parent.mkdir(parents=True, exist_ok=True)
    dest.write_text(render_monetize_html(state), encoding="utf-8")
    return dest
...
def publish_static_monetize(
    output_dir: str = DEFAULT_OUTPUT_DIR,
    fallback_path: Optional[str] = None,
) -> Dict[str, str]:
    """Write output/monetize.html plus the committed reports fallback."""
    live = write_monetize_html(output_dir)
    fallback = write_static_monetize_html(fallback_path)
    return {
        "monetize_html": str(live.resolve()),
        "static_html": str(fallback.resolve()),
    }
...
def preview_base_url() -> Optional[str]:
    for key in ("EW_PREVIEW_URL", "CURSOR_PREVIEW_URL", "VSCODE_PROXY_URI"):
        raw = (os.environ.get(key) or "").strip()
        if raw:
            return raw.rstrip("/")
    host = (os.environ.get("EW_PREVIEW_HOST") or "").strip()
    if not host:
        return None
    if "://" not in host:
        host = "https://" + host
...
def explorer_launch_urls(host: str, port: int, path: str = "/monetize") -> List[str]:
    """Localhost + LAN/public URLs so a human can click one that reaches this process."""
    if not path.startswith("/"):
        path = "/" + path
    urls = [f"http://127.0.0.1:{port}{path}"]
    ...
    if preview:
        resolved = preview.replace("{{port}}", str(port)).replace("{port}", str(port))
        urls.append(resolved if resolved.endswith(path) else resolved + path)
    seen: set[str] = set()
    out: List[str] = []
    for url in urls:
        if url not in seen:
...
def print_explorer_launch(host: str, port: int, path: str = "/monetize") -> None:
    print("[monetize-ui] Open the Monetize Explorer:")
    print()
    for url in explorer_launch_urls(host, port, path):
        print(url)
        print()
    print(f"[monetize-ui] Status API: http://127.0.0.1:{port}/api/monetize/status")
    print(f"[monetize-ui] Bound to {host}:{port}")
    print("[monetize-ui] No clickable preview? Write a local file instead:")
    print("[monetize-ui]   python3 ew_tool.py --monetize-ui --static")


def print_static_launch(paths: Mapping[str, str]) -> None:
    print("[monetize-ui] Self-contained Monetize Explorer (open this file on your machine):")
    print()
    for key in ("static_html", "monetize_html"):
        if key in paths:
            print(file_url(paths[key]))
            print()
    print("[monetize-ui] Or open the committed fallback in the repo:")
    print("[monetize-ui]   reports/monetize_explorer.html")
...
def dispatch_monetize(
    ...
    *,
    ...
    if method == "GET" and path in html_paths:
        encoded = render_monetize_html().encode("utf-8")
        return 200, {
...
EW_LICENSE_TIER=free|pro|enterprise</pre>
  ...
  <script>
    ...
    function stateForTier(base, tier) {
      const t = tier || (base.license && base.license.tier) || "free";
      const snapshot = (base.tiers || {})[t] || {};
      const features = (base.features || []).map(function(row) {
        const on = row.allowed_on || {};
        return Object.assign({}, row, {allowed: !!on[t]});
      });
      const demo = (base.demo_features || []).map(function(f) {
        const row = features.find(function(r) { return r.key === f.key; });
        return Object.assign({}, f, {allowed: !!(row && row.allowed)});
      });
      return Object.assign({}, base, {
        license: Object.assign({}, base.license || {}, {
          tier: snapshot.tier || t,
        ...
        features: features,
        demo_features: demo,
        preview: t !== base.process_tier
      });
    }

    function requireFromState(feature) {
      const row = (state && state.features || []).find(function(f) { return f.key === feature; });
      const allowed = !!(row && row.allowed);
      const tier = (state && state.license && state.license.tier) || "free";
      if (allowed) {
        return {
          ok: true,
          feature: feature,
          tier: tier,
          exit_code: 0,
          message: "Feature '" + feature + "' is allowed on the '" + tier + "' tier."
        };
      }
      const minTier = (row && row.min_tier) || "pro";
      return {
        ok: false,
        feature: feature,
        tier: tier,
        exit_code: 2,
        error: "Feature '" + feature + "' requires the '" + minTier + "' tier (current: '" + tier + "').",
        hint: "Upgrade via EW_LICENSE_TIER=pro or EW_LICENSE_TIER=enterprise. See --monetize-status."
      };
    }
    ...
    function renderCli(d) {
      ...
      document.getElementById("cli-help").textContent =
        ...
        (c.static || "python3 ew_tool.py --monetize-ui --static") + "\\n" +
        (c.env || "EW_LICENSE_TIER=free|pro|enterprise");
    }

    function showRequire(d, feature) {
    ...
    function render(d) {
      ...
      if (d.warning) {
        ...
        warn.hidden = false;
        warn.textContent = "Offline explorer — Free / Pro / Enterprise switch uses the embedded access matrix. No server required.";
      } else {
        warn.hidden = true;
      }
      const mon = document.querySelector('.nav a[href="/monitor"]');
      if (mon) mon.hidden = !isHttp();
      renderCards(d);
      renderSwitcher(d);
      renderMatrix(d);
      renderRoyalty(d);
    ...
    function renderEmbedded(tier) {
      const embedded = embeddedState();
      if (!embedded) return false;
      render(stateForTier(embedded, tier || qsTier() || (embedded.license && embedded.license.tier)));
      return true;
    }
    ...
    async function load(tier) {
      ...
      if (isHttp()) {
        try {
          ...
          if (renderEmbedded(tier)) return;
          err.hidden = false;
          err.textContent = "Failed to load Monetize Explorer: " + e.message +
            ". Generate an offline copy with: python3 ew_tool.py --monetize-ui --static";
      ...
      if (renderEmbedded(tier)) return;
      err.hidden = false;
      err.textContent = "No embedded access matrix. Generate with: python3 ew_tool.py --monetize-ui --static";
    }
    ...
    async function previewTier(tier) {
      ...
      } catch (e) { /* file:// without a host can reject URL mutation */ }
      await load(tier);
    }
    ...
    async function applyTier(tier) {
      if (isHttp()) {
        try {
          ...
          render(d);
          return;
        } catch (e) { /* fall back to embedded preview */ }
      }
      if (renderEmbedded(tier)) {
        const note = document.getElementById("tier-note");
        note.innerHTML = (note.innerHTML || "") +
          ' <span class="preview-note">Apply-to-process needs the local server; this is a client-side preview.</span>';
      }
    }
    ...
    async function tryFeature(feature) {
      ...
      if (isHttp()) {
        try {
          ...
          showRequire(await res.json(), feature);
          return;
        } catch (e) { /* fall back */ }
      }
      showRequire(requireFromState(feature), feature);
    }
