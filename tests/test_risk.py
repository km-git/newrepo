"""Tests for dynamic DCA and stop-loss sanity."""

from __future__ import annotations

import pytest

from core.risk import build_dca_ladder, dynamic_stop, stop_is_sane
...
def test_dca_splits_10_20_30_40():
  ...
  assert legs[1]["price"] <= 100.0
  assert legs[2]["price"] <= legs[1]["price"]
  assert legs[3]["price"] <= legs[2]["price"]


def test_dca_ignores_htf_fibs_outside_zone():
  """HTF fib cluster must not pull DCA legs away from anchor."""
  legs = build_dca_ladder(
    "SHORT", 73468.0, 500.0, 72842.0, 74094.0,
    fib_levels=[110367.0, 100572.0, 84739.0],
  )
  assert legs[1]["price"] <= 74094.0
  assert legs[1]["price"] >= 73468.0
  assert legs[2]["price"] >= legs[1]["price"]
...
def test_dynamic_stop_short_clamps_stale_structure():
  """Stale HTF high must not produce absurd short stop."""
  s = dynamic_stop(
