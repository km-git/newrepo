...
def _parse_rate_card(spec: str) -> RateCard:
    ...
    if len(parts) not in (3, 4):
        raise RoyaltyError(f"rate card must be LICENSE_ID:MODEL:RATE[:CURRENCY], got {spec!r}")
    try:
...
def _cmd_report(args: argparse.Namespace) -> int:
    ...
    report = compute_royalty_report(ledger, licenses, cards, period_start=args.start, period_end=args.end)
    if args.json:
        _print_json(report)
    else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_tag.add_argument("--right", dest="rights", action="append", help="right granted (repeatable)")
    p_tag.add_argument("--territory", default="worldwide")
    p_tag.add_argument("--expires-at", default=None, help="ISO-8601 UTC expiry (omit for perpetual)")
    p_tag.add_argument("--attribution-required", action="store_true")
    p_tag.add_argument("--manifest-sha256", required=True, help="SHA-256 of the asset's source sidecar manifest")
    ...
    p_policy.add_argument("--prefix", action="append", help="asset prefix covered by the policy (repeatable)")
    p_policy.add_argument("--aws-bucket", default=None, help="render as AWS-S3-style policy for this bucket")
    p_policy.add_argument(
        "--revoke", metavar="POLICY_ID", default=None, help="revoke an existing policy instead of issuing"
    )
