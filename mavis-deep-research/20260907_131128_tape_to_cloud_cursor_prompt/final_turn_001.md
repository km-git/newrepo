...
Save as `.cursor/rules/tape-to-cloud-build.mdc` at the repo root. `Apply Intelligently` means Cursor's agent reads the description and pulls the rule in whenever the chat looks like work on this tool.
...
2. **Target-agnostic.** Same code path writes to AWS S3/Glacier/Deep Archive, Azure Blob (Hot/Cool/Archive), Google Cloud Storage, Backblaze B2, Wasabi, Cloudian, MinIO, Ceph. Use any S3-compatible SDK; do not hard-code to one cloud.
